...
uint8_t GRBLCode::execute_line(char *line)
{
  /* -------------------------------------------------------------------------------------
  ...
  memset(&block, 0, sizeof(parser_block_t)); // Initialize the parser block struct.
  memcpy(&block.modal,&state.modal,sizeof(gc_modal_t)); // Copy current modes
  ...
  if (line[0] == '$') { // NOTE: `$J=` already parsed when passed to this function.
	...
	block.modal.motion = MOTION_MODE_LINEAR;
    block.modal.feed_rate = FEED_RATE_MODE_UNITS_PER_MIN;
#ifdef USE_LINE_NUMBERS
     block.values.n = JOG_LINE_NUMBER; // Initialize default line number reported during jog.
#endif
  }
    ...
    switch(letter) {
      ...
      case 'G':
        ...
        switch(int_value) {
          ...
          case 4: case 53:
            ...
            block.non_modal_command = int_value;
            if ((int_value == 28) || (int_value == 30) || (int_value == 92)) {
              if (!((mantissa == 0) || (mantissa == 10))) { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); }
              block.non_modal_command += mantissa;
          ...
          case 80:
            ...
            block.modal.motion = int_value;
            if (int_value == 38){
              if (!((mantissa == 20) || (mantissa == 30) || (mantissa == 40) || (mantissa == 50))) {
                FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G38.x command]
              }
              block.modal.motion += (mantissa/10)+100;
              mantissa = 0; // Set to zero to indicate valid non-integer G command.
            }  
            break;
          case 17: case 18: case 19:
            word_bit = MODAL_GROUP_G2;
            block.modal.plane_select = int_value - 17;
            break;
          case 90: case 91:
            if (mantissa == 0) {
              word_bit = MODAL_GROUP_G3;
              block.modal.distance = int_value - 90;
            } else {
          ...
          case 93: case 94:
            ...
            block.modal.feed_rate = 94 - int_value;
            break;
          case 20: case 21:
            word_bit = MODAL_GROUP_G6;
            block.modal.units = 21 - int_value;
            break;
          case 40:
            word_bit = MODAL_GROUP_G7;
            // NOTE: Not required since cutter radius compensation is always disabled. Only here
            // to support G40 commands that often appear in g-code program headers to setup defaults.
            // block.modal.cutter_comp = CUTTER_COMP_DISABLE; // G40
            break;
          case 43: case 49:
            ...
            if (int_value == 49) { // G49
              block.modal.tool_length = TOOL_LENGTH_OFFSET_CANCEL;
            } else if (mantissa == 10) { // G43.1
              block.modal.tool_length = TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC;
          ...
          case 54: case 55: case 56: case 57: case 58: case 59:
            ...
            block.modal.coord_select = int_value - 54; // Shift to array indexing.
            break;
          case 61:
            word_bit = MODAL_GROUP_G13;
            if (mantissa != 0) { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G61.1 not supported]
            // block.modal.control = CONTROL_MODE_EXACT_PATH; // G61
            break;
      ...
      case 'M':
        ...
        switch(int_value) {
          case 0: case 1: case 2: case 30:
            ...
            switch(int_value) {
              case 0: block.modal.program_flow = PROGRAM_FLOW_PAUSED; break; // Program pause
              case 1: break; // Optional stop not supported. Ignore.
              default: block.modal.program_flow = int_value; // Program end and reset
            }
            break;
            ...
            switch(int_value) {
              #ifdef ENABLE_M7
                case 7: block.modal.coolant = COOLANT_MIST_ENABLE; break;
              #endif
            ...
            break;
						...
						case 56:
							...
							block.modal.override = OVERRIDE_PARKING_MOTION;
							break;
					#endif
      ...
      default:
        ...
        switch(letter){
          ...
          case 'F': word_bit = WORD_F; block.values.f = value; break;
          // case 'H': // Not supported
          case 'I': word_bit = WORD_I; block.values.ijk[X_AXIS] = value; ijk_words |= (1<<X_AXIS); break;
          case 'J': word_bit = WORD_J; block.values.ijk[Y_AXIS] = value; ijk_words |= (1<<Y_AXIS); break;
          case 'K': word_bit = WORD_K; block.values.ijk[Z_AXIS] = value; ijk_words |= (1<<Z_AXIS); break;
          case 'L': word_bit = WORD_L; block.values.l = int_value; break;
          case 'N': word_bit = WORD_N; block.values.n = truncf(value); break;
          case 'P': word_bit = WORD_P; block.values.p = value; break;
          // NOTE: For certain commands, P value must be an integer, but none of these commands are supported.
          // case 'Q': // Not supported
          case 'R': word_bit = WORD_R; block.values.r = value; break;
          case 'S': word_bit = WORD_S; block.values.s = value; break;
		  case 'T': word_bit = WORD_T;
				if (value > MAX_TOOL_NUMBER) { FAIL(STATUS_GCODE_MAX_VALUE_EXCEEDED); }
					block.values.t = int_value;
				break;
		  case 'X': word_bit = WORD_X; block.values.xyz[X_AXIS] = value; axis_words |= (1<<X_AXIS); break;
          case 'Y': word_bit = WORD_Y; block.values.xyz[Y_AXIS] = value; axis_words |= (1<<Y_AXIS); break;
          case 'Z': word_bit = WORD_Z; block.values.xyz[Z_AXIS] = value; axis_words |= (1<<Z_AXIS); break;
  ...
  */
  ...
  if (bit_istrue(value_words,bit(WORD_N))) {
    ...
    if (block.values.n > MAX_LINE_NUMBER) { FAIL(STATUS_GCODE_INVALID_LINE_NUMBER); } // [Exceeds max line number]
  }
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    ...
    if (block.modal.units == UNITS_MODE_INCHES) { block.values.f *= MM_PER_INCH; }
  } else {
    if (block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) { // = G93
      // NOTE: G38 can also operate in inverse time, but is undefined as an error. Missing F word check added here.
      if (axis_command == AXIS_COMMAND_MOTION_MODE) {
		if ((block.modal.motion != MOTION_MODE_NONE) && (block.modal.motion != MOTION_MODE_SEEK)) {
      ...
      if (state.modal.feed_rate == FEED_RATE_MODE_UNITS_PER_MIN) { // Last state is also G94
        if (bit_istrue(value_words,bit(WORD_F))) {
          if (block.modal.units == UNITS_MODE_INCHES) { block.values.f *= MM_PER_INCH; }
        } else {
          block.values.f = state.feed_rate; // Push last state feed rate
        }
  ...
  if (bit_isfalse(value_words,bit(WORD_S))) { block.values.s = state.spindle_speed; }
  ...
  // [8. Coolant control ]: N/A
	...
	if (bit_istrue(command_words, bit(MODAL_GROUP_M9))) { // Already set as enabled in parser.
		if (bit_istrue(value_words, bit(WORD_P))) {
			if (block.values.p == 0.0f) { block.modal.override = OVERRIDE_DISABLED; }
  ...
  if (block.non_modal_command == NON_MODAL_DWELL) {
    if (bit_isfalse(value_words,bit(WORD_P))) { FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } // [P word missing]
    bit_false(value_words,bit(WORD_P));
  }

  // [11. Set active plane ]: N/A
  switch (block.modal.plane_select) {
  ...
  uint8_t idx;
  if (block.modal.units == UNITS_MODE_INCHES) {
  ...
  if (axis_command == AXIS_COMMAND_TOOL_LENGTH_OFFSET ) { // Indicates called in block.
    if (block.modal.tool_length == TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC) {
  ...
  memcpy(block_coord_system,state.coord_system,sizeof(state.coord_system));
      ...
      for (idx=0; idx<N_AXIS; idx++) { // Axes indices are consistent, so loop may be used.
        ...
        if (bit_istrue(axis_words,bit(idx)) ) {
          if (block.values.l == 20) {
            ...
            block.values.ijk[idx] = state.position[idx]-state.coord_offset[idx]-block.values.xyz[idx];
            if (idx == TOOL_LENGTH_OFFSET_AXIS) { block.values.ijk[idx] -= state.tool_length_offset; }
          } else {
    ...
    case NON_MODAL_SET_COORDINATE_OFFSET:
      ...
      for (idx=0; idx<N_AXIS; idx++) { // Axes indices are consistent, so loop may be used.
        if (bit_istrue(axis_words,bit(idx)) ) {
          ...
          block.values.xyz[idx] = state.position[idx]-block_coord_system[idx]-block.values.xyz[idx];
          if (idx == TOOL_LENGTH_OFFSET_AXIS) { block.values.xyz[idx] -= state.tool_length_offset; }
        } else {
          block.values.xyz[idx] = state.coord_offset[idx];
        }
      }
      break;
    ...
    default:
        ...
        if (axis_words) {
          for (idx=0; idx<N_AXIS; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
            if ( bit_isfalse(axis_words,bit(idx)) ) {
              block.values.xyz[idx] = state.position[idx]; // No axis word in block. Keep same axis position.
            } else {
              // Update specified value according to distance mode or ignore if absolute override is active.
              // NOTE: G53 is never active with G28/30 since they are in the same modal group.
              if (block.non_modal_command != NON_MODAL_ABSOLUTE_OVERRIDE) {
                // Apply coordinate offsets based on distance mode.
                if (block.modal.distance == DISTANCE_MODE_ABSOLUTE) {
                  block.values.xyz[idx] += block_coord_system[idx] + state.coord_offset[idx];
                  if (idx == TOOL_LENGTH_OFFSET_AXIS) { block.values.xyz[idx] += state.tool_length_offset; }
                } else {  // Incremental mode
                  block.values.xyz[idx] += state.position[idx];
                }
              }
            }
          }
      ...
      switch (block.non_modal_command) {
        ...
        case NON_MODAL_GO_HOME_1: // G30
          ...
          if (axis_words) {
            ...
            for (idx=0; idx<N_AXIS; idx++) {
              if (!(axis_words & (1<<idx))) { block.values.ijk[idx] = state.position[idx]; }
            }
          } else {
        ...
        case NON_MODAL_ABSOLUTE_OVERRIDE:
          ...
          if (!(block.modal.motion == MOTION_MODE_SEEK || block.modal.motion == MOTION_MODE_LINEAR)) {
  ...
  // [20. Motion modes ]:
  if (block.modal.motion == MOTION_MODE_NONE) {
  ...
  // was explicitly commanded in the g-code block.
    ...
    if (block.modal.motion == MOTION_MODE_SEEK) {
    ...
    // the value must be positive. In inverse time mode, a positive value must be passed with each block.
      ...
      if (block.values.f == 0.0f) { FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [Feed rate undefined]
      ...
      switch (block.modal.motion) {
        ...
        case MOTION_MODE_CCW_ARC:
          ...
          float x,y;
          x = block.values.xyz[axis_0]-state.position[axis_0]; // Delta x between current position and target
          y = block.values.xyz[axis_1]-state.position[axis_1]; // Delta y between current position and target

          if (value_words & bit(WORD_R)) { // Arc Radius Mode
            bit_false(value_words,bit(WORD_R));
            if (isequal_position_vector(state.position, block.values.xyz)) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Invalid target]

            // Convert radius value to proper units.
            if (block.modal.units == UNITS_MODE_INCHES) { block.values.r *= MM_PER_INCH; }
            ...
            */
            ...
            float h_x2_div_d = 4.0f * block.values.r*block.values.r - x*x - y*y;
            ...
            if (block.modal.motion == MOTION_MODE_CCW_ARC) { h_x2_div_d = -h_x2_div_d; }
            ...
            */
            ...
            block.values.ijk[axis_0] = 0.5f*(x-(y*h_x2_div_d));
            block.values.ijk[axis_1] = 0.5f*(y+(x*h_x2_div_d));
            ...
            if (block.modal.units == UNITS_MODE_INCHES) {
              for (idx=0; idx<N_AXIS; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
                if (ijk_words & bit(idx)) { block.values.ijk[idx] *= MM_PER_INCH; }
              }
            }

            // Arc radius from center to target
            x -= block.values.ijk[axis_0]; // Delta x between circle center and target
            y -= block.values.ijk[axis_1]; // Delta y between circle center and target
            float target_r = hypot_f(x,y);

            // Compute arc radius for mc_arc. Defined from current location to center.
            block.values.r = hypot_f(block.values.ijk[axis_0], block.values.ijk[axis_1]);

            // Compute difference between current location and target radii for final error-checks.
            float delta_r = fabsf(target_r-block.values.r);
            if (delta_r > 0.005f) {
              if (delta_r > 0.5f) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.5mm
              if (delta_r > (0.001f*block.values.r)) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.005mm AND 0.1% radius
            }
          }
          break;
        ...
        case MOTION_MODE_PROBE_TOWARD: case MOTION_MODE_PROBE_AWAY:
            if ((block.modal.motion == MOTION_MODE_PROBE_AWAY) ||
                (block.modal.motion == MOTION_MODE_PROBE_AWAY_NO_ERROR)) { gc_parser_flags |= GC_PARSER_PROBE_IS_AWAY; }
  ...
  */
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
      ...
      pl_data->spindle_speed = state.spindle_speed;
      plan_data.condition = (state.modal.spindle | state.modal.coolant);

      uint8_t status = jog_execute(&plan_data, &block);
      if (status == STATUS_OK) { memcpy(state.position, block.values.xyz, sizeof(block.values.xyz)); }
      return(status);
  }

  // If in laser mode, setup laser power based on current and past parser conditions.
  if (bit_istrue(grbl.settings.flags(), BITFLAG_LASER_MODE)) {
      if (!((block.modal.motion == MOTION_MODE_LINEAR) || (block.modal.motion == MOTION_MODE_CW_ARC)
          || (block.modal.motion == MOTION_MODE_CCW_ARC))) {
      ...
      if (axis_words && (axis_command == AXIS_COMMAND_MOTION_MODE)) {
        ...
        if (state.modal.spindle == SPINDLE_ENABLE_CW) {
          if ((state.modal.motion == MOTION_MODE_LINEAR) || (state.modal.motion == MOTION_MODE_CW_ARC)
            || (state.modal.motion == MOTION_MODE_CCW_ARC)) {
  ...
  #ifdef USE_LINE_NUMBERS
    pl_data->line_number = state.line_number; // Record data for planner use.
  #endif
  ...
  if (state.modal.feed_rate) { pl_data->condition |= PL_COND_FLAG_INVERSE_TIME; } // Set condition flag for planner use.

  // [3. Set feed rate ]:
  state.feed_rate = block.values.f; // Always copy this value. See feed rate error-checking.
  pl_data->feed_rate = state.feed_rate; // Record data for planner use.

  // [4. Set spindle speed ]:
  if ((state.spindle_speed != block.values.s) || bit_istrue(gc_parser_flags, GC_PARSER_LASER_FORCE_SYNC)) {
      if (state.modal.spindle != SPINDLE_DISABLE) {
#ifdef VARIABLE_SPINDLE
        if (bit_isfalse(gc_parser_flags, GC_PARSER_LASER_ISMOTION)) {
          if (bit_istrue(gc_parser_flags, GC_PARSER_LASER_DISABLE)) {
            grbl.spindle.sync(state.modal.spindle, 0.0);
          }
          else { grbl.spindle.sync(state.modal.spindle, block.values.s); }
        }
#else
          spindle_sync(state.modal.spindle, 0.0);
#endif
      }
      state.spindle_speed = block.values.s; // Update spindle speed state.
  }
  // NOTE: Pass zero spindle speed for all restricted laser motions.
  if (bit_isfalse(gc_parser_flags, GC_PARSER_LASER_DISABLE)) {
      pl_data->spindle_speed = state.spindle_speed; // Record data for planner use. 
  } // else { pl_data->spindle_speed = 0.0; } // Initialized as zero already.

  // [5. Select tool ]: NOT SUPPORTED. Only tracks tool value.
  state.tool = block.values.t;

  // [6. Change tool ]: NOT SUPPORTED

    // [7. Spindle control ]:
    if (state.modal.spindle != block.modal.spindle) {
        // Update spindle control and apply spindle speed when enabling it in this block.
        // NOTE: All spindle state changes are synced, even in laser mode. Also, pl_data,
        // rather than gc_state, is used to manage laser state for non-laser motions.
        grbl.spindle.sync(block.modal.spindle, pl_data->spindle_speed);
...
#ifdef ENABLE_PARKING_OVERRIDE_CONTROL
	if (state.modal.override != block.modal.override) {
		state.modal.override = block.modal.override;
		mc_override_ctrl_update(state.modal.override);
	}
#endif

  // [10. Dwell ]:
  if (block.non_modal_command == NON_MODAL_DWELL) { grbl.motion.dwell(block.values.p); }

  // [11. Set active plane ]:
  state.modal.plane_select = block.modal.plane_select;

  // [12. Set length units ]:
  state.modal.units = block.modal.units;

  // [13. Cutter radius compensation ]: G41/42 NOT SUPPORTED
  // state.modal.cutter_comp = block.modal.cutter_comp; // NOTE: Not needed since always disabled.
  ...
  if (axis_command == AXIS_COMMAND_TOOL_LENGTH_OFFSET ) { // Indicates a change.
    state.modal.tool_length = block.modal.tool_length;
    if (state.modal.tool_length == TOOL_LENGTH_OFFSET_CANCEL) { // G49
  ...
  // state.modal.control = block.modal.control; // NOTE: Always default.
  ...
  switch(block.non_modal_command) {
    ...
    case NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1:
      ...
      if (axis_command) { grbl.motion.line(block.values.xyz, pl_data); }
      grbl.motion.line(block.values.ijk, pl_data);
  ...
  if (state.modal.motion != MOTION_MODE_NONE) {
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
      uint8_t gc_update_pos = GC_UPDATE_POS_TARGET;
      if (state.modal.motion == MOTION_MODE_LINEAR) {
          grbl.motion.line(block.values.xyz, pl_data);
      } else if (state.modal.motion == MOTION_MODE_SEEK) {
          pl_data->condition |= PL_COND_FLAG_RAPID_MOTION; // Set rapid motion condition flag.
          grbl.motion.line(block.values.xyz, pl_data);
      } else if ((state.modal.motion == MOTION_MODE_CW_ARC) || (state.modal.motion == MOTION_MODE_CCW_ARC)) {
          grbl.motion.arc(block.values.xyz, pl_data, state.position, block.values.ijk, block.values.r,
              axis_0, axis_1, axis_linear, bit_istrue(gc_parser_flags, GC_PARSER_ARC_IS_CLOCKWISE));
      } else {
        // NOTE: block.values.xyz is returned from mc_probe_cycle with the updated position value. So
        // upon a successful probing cycle, the machine position and the returned value should be the same.
        #ifndef ALLOW_FEED_OVERRIDE_DURING_PROBE_CYCLES
          pl_data->condition |= PL_COND_FLAG_NO_FEED_OVERRIDE;
        #endif
        gc_update_pos = grbl.motion.probe_cycle(block.values.xyz, pl_data, gc_parser_flags);
    }  
  ...
  if (state.modal.program_flow) {
    ...
    if (state.modal.program_flow == PROGRAM_FLOW_PAUSED) {
      ...
      state.modal.motion = MOTION_MODE_LINEAR;
      state.modal.plane_select = PLANE_SELECT_XY;
      state.modal.distance = DISTANCE_MODE_ABSOLUTE;
      state.modal.feed_rate = FEED_RATE_MODE_UNITS_PER_MIN;
      // state.modal.cutter_comp = CUTTER_COMP_DISABLE; // Not supported.
      state.modal.coord_select = 0; // G54
      state.modal.spindle = SPINDLE_DISABLE;
      state.modal.coolant = COOLANT_DISABLE;
			#ifdef ENABLE_PARKING_OVERRIDE_CONTROL
				#ifdef DEACTIVATE_PARKING_UPON_INIT
					state.modal.override = OVERRIDE_DISABLED;
				#else
					state.modal.override = OVERRIDE_PARKING_MOTION;
				#endif
			#endif
